...
document.getElementById('currentPrice').textContent = product.price.toLocaleString()+' EGP';
...
if (product.discount) {
    document.getElementById('badgeMain').textContent = product.discount + ' OFF';
} else {
    document.getElementById('badgeMain').style.display = 'none';
}
...
function renderRelated() {
    ...
    document.getElementById('relatedGrid').innerHTML = related.map(p=>`
        <div class="related-card" onclick="window.location.href='productDetails.html?id=${p.id}'">
            <img class="r-img" src="${p.img}" alt="${p.name}">
            <div class="r-info">
                <h4>${p.name}</h4>
                <p class="r-price">${p.price.toLocaleString()} EGP</p>
